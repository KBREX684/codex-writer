...
def test_write_pipeline_records_pipeline_steps_and_agent_runs(tmp_path):
    project = tmp_path / "book"
    run_cli("init", "--project-root", str(project), "--title", "书", "--genre", "修仙", "--format", "json")
    run_cli("plan", "--project-root", str(project), "--chapter", "1", "--title", "入局", "--demo", "--format", "json")
    result = run_cli("write", "--project-root", str(project), "--chapter", "1", "--demo", "--format", "json")
    ...
    # polish_agent step was removed; pipeline now goes reviewed → extracted
    assert ("reviewed", "extracted", "extract_agent") in transitions
    ...
    assert {"draft_agent", "review_agent", "extract_agent"} <= agents
